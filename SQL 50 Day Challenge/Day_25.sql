...
customers table : id, name phone address 
orders table :order_id, cxid order_date and cost
*/
...
INSERT INTO customer (id, first_name, last_name, city, address, phone_number) VALUES
...
select count(address) shippable_orders
	,count(id) total_order
	,ROUND(count(address)*1.0/ count(id) *100,2) pct_shipable_orders
from(
select c.id cust_id, c.address, o.*
from customer c
JOIN orders o
ON c.id = o.cust_id
order by o.id
...
/*
Find out the percentage of orders where customer doesn't have valid phone numbers 
...
select 
	SUM(CASE WHEN length(phone_number)!=10 THEN 1 ELSE 0 END) invalid_phone_numbers
	,count(*) total_ordes
	,ROUND((SUM(CASE WHEN length(phone_number)!=10 THEN 1 ELSE 0 END)::numeric
	/ count(*)::numeric)*100,2) pct_invalid_phone_no_orders
from orders o
JOIN customer c
ON o.cust_id = c.id;
