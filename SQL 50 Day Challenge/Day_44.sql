...
/*
Write a query to identify properites where the mkt_price of the house 
exceeds the city's average mkt_price.
 house_price table: id, state, city, street_address, mkt_price
*/
...
from(
	select *
		,avg(mkt_price) over(partition by city) avg_price_per_city
	from house_price
)
where mkt_price > avg_price_per_city;
...
from(
	select *
		,avg(mkt_price) over(partition by city) city_avg_price
		,avg(mkt_price) over() country_avg_price
	from house_price
)
where mkt_price > city_avg_price and mkt_price < country_avg_price


--solution using correalted subquery
select * 
from house_price h1
where h1.mkt_price > (SELECT avg(h2.mkt_price) 
		from house_price h2
		where h2.city = h1.city)
	and h1.mkt_price < (select avg(mkt_price) from house_price)
