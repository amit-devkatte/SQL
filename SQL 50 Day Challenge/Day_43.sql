--Day 43
/*
Write a query to find out each country's most profitable comapany details
*/

select country, company, sector, industry, sales, profits, rank
from (
	select *
		, rank() over(partition by country order by profits desc) as com_rnk
	from forbes_global
)
where com_rnk = 1;
...
SELECT sector,company, country, industry, sales, profits, rank
FROM(
	select * ,
		dense_rank() over(partition by sector order by profits) sec_rnk
	from forbes_global
)
WHERE sec_rnk <=2;
