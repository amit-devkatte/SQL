...
Expected Output: Category: Name of the product category.
...
select r.*
	,p.category
	,count(return_id) over(partition by category) total_return_by_category
	,count(return_id) over() as total_overall_return
from return_records r
left join amazon_products p
on p.product_id = r.product_id
)
select category
	,ROUND(total_return_by_category*1.0/total_overall_return *100,2) as Return_Percentage
from cte
GROUP by 1,2
...
select p.category
	,ROUND(count(return_id)* 1.0/ (select count(return_id) from return_records)*100,2) return_percentage
from return_records r
left join amazon_products p
