--Day 27
/*
find the best selling item for each month (no need to seperate month and year) where the biggest total
invoice was paid.
the best sellling item is calculated using the formula (unitprice * quantity).
...
select month, description, invoice
from(
select 
	date_part('month', invoicedate) as month, 
	description
	,SUM(unitprice* quantity) as invoice 
...
/*
Find Customer of the month from each MONTH one customer who has spent the highest amount (price * quantity) 
as total amount may include multiple purchase
*/

select month, customerid, total_amount
from(
	select extract('month' from invoicedate) as month
		...
		,rank() over(partition by extract('month' from invoicedate)
					order by sum(unitprice * quantity) desc) rnk
	from walmart_eu
	group by 1,2
)
where rnk =1;
