/*Q22. Flipkart business analyst sql interview question
Identify the region with the lowest sales amount for the previous
month.
-- group by region and months
-- filter by last MONTH
-- order by asc and limit 1 or use min function on amount
sales table : region, sale_date, amount
*/
-------------------------------------------------------------
-- Solution
-------------------------------------------------------------
...
SELECT 
	...
	RANK() OVER (ORDER BY sum(amount) ASC) as rnk 
FROM sales
WHERE 
	EXTRACT (MONTH from saleDate)=EXTRACT (month from current_date - interval '1 month') 
	AND
